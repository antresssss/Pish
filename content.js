...
// Global variable to track scanning status
let isScanning = false; // Initialize as false and wait for actual state

// Function to get the current scanning state from storage
async function initializeScanningState() {
  try {
    const result = await chrome.storage.local.get('isScanning');
    isScanning = result.isScanning !== undefined ? result.isScanning : true;
    
    // If scanning is enabled, start the initial scan
    if (isScanning) {
      scanForUrls();
    }
  } catch (error) {
    console.error('Error getting scanning state:', error);
  }
}
...
// Initialize the scanning state when the content script loads
